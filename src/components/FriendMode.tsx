...
import { AccountabilityBuddy } from './AccountabilityBuddy';
...
export function FriendMode({ tasks, isOpen, onClose }: FriendModeProps) {
  ...
  const [activeTab, setActiveTab] = useState<'share' | 'buddy'>('share');
  const [showAccountabilityBuddy, setShowAccountabilityBuddy] = useState(false);
  ...
  return (
    <>
      <div className="fixed inset-0 modal-backdrop z-50 flex items-center justify-center p-4">
        <div className="glass-card rounded-2xl p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto animate-slide-up">
          ...
          <div className="flex gap-1 mb-6 bg-gray-100 dark:bg-gray-800 rounded-lg p-1">
            ...
            <button
              onClick={() => setShowAccountabilityBuddy(true)}
              className="flex items-center gap-2 px-4 py-2 rounded-lg font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-all"
            >
      ...
      {/* Accountability Buddy Modal */}
      <AccountabilityBuddy
        tasks={tasks}
        isOpen={showAccountabilityBuddy}
        onClose={() => setShowAccountabilityBuddy(false)}
      />
    </>
  );
}
